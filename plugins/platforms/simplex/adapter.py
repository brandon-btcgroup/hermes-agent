...
Optional environment variables:
    ...
    SIMPLEX_FILE_DIR           Host directory mirrored to the daemon's
                               files folder via bind-mount. Required for
                               outbound media (image/voice/video/file).
                               Unset = text-only outbound; image_url
                               messages degrade to a plain text URL.
    SIMPLEX_DAEMON_FILES_FOLDER Container-side path of the daemon's
                               files folder (default
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        # Bind-mount mapping for outbound media. When SIMPLEX_FILE_DIR is
        # unset, all outbound media falls back to text-only (image_url
        # → URL in body). When set, files are staged into this dir under
        # the host view and the daemon is told the equivalent container
        # path. Defaults mirror the typical docker deployment.
