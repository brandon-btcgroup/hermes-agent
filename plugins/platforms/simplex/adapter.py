...
def check_requirements() -> bool:
    """Return True iff this Python environment can run the SimpleX adapter.

    The protocol layer uses ``websockets`` (a third-party package) — if it
    isn't importable the adapter won't work at all, regardless of config.
    """
    try:
...
def validate_config(config) -> bool:
    """Return True iff ``config`` carries enough to connect to a daemon.

    Validates structure, not reachability; a bad WS URL is detected at
    connect time and surfaced via the supervisor's reconnect loop.
    """
...
def is_connected(config) -> bool:
    """Return True iff SimpleX is configured (env or config.yaml)."""
    extra = getattr(config, "extra", {}) or {}
    ws_url = os.getenv("SIMPLEX_WS_URL") or extra.get("ws_url", "")
    group_ids = extra.get("group_ids") or [
        g for g in (os.getenv("SIMPLEX_GROUP_IDS", "") or "").split(",") if g.strip()
    ]
    return bool(ws_url and group_ids)


def _apply_env(config) -> None:
    """Hydrate ``config.extra`` and ``home_channel`` from SIMPLEX_* env vars.

    Called from the adapter factory so a user with only ``SIMPLEX_WS_URL`` /
    ``SIMPLEX_GROUP_IDS`` set in ``~/.hermes/.env`` gets a working adapter
    without writing config.yaml by hand.
    """
    from gateway.config import HomeChannel, Platform

    if not getattr(config, "extra", None):
        config.extra = {}
    extra = config.extra

    ws_url = os.getenv("SIMPLEX_WS_URL")
    if ws_url and not extra.get("ws_url"):
        extra["ws_url"] = ws_url

    raw_groups = os.getenv("SIMPLEX_GROUP_IDS", "")
    if raw_groups and not extra.get("group_ids"):
        extra["group_ids"] = [g.strip() for g in raw_groups.split(",") if g.strip()]

    if "max_reconnect_delay_s" not in extra:
        extra["max_reconnect_delay_s"] = int(os.getenv("SIMPLEX_MAX_RECONNECT_DELAY_S", "60"))
    file_dir = os.getenv("SIMPLEX_FILE_DIR", "").strip()
    if file_dir and not extra.get("file_dir"):
        extra["file_dir"] = file_dir
    if "daemon_files_folder" not in extra:
        extra["daemon_files_folder"] = os.getenv(
            "SIMPLEX_DAEMON_FILES_FOLDER", "/root/.simplex/files"
        )

    home = os.getenv("SIMPLEX_HOME_GROUP_ID")
    if home and not getattr(config, "home_channel", None):
...
def register(ctx) -> None:
    ...
    from .cli import add_simplex_subparser
    ...
    ctx.register_platform(
        ...
        install_hint="pip install hermes-agent[simplex]  # or: pip install websockets",
        ...
        # Display
        emoji="🛡️",
        # SimpleX has no phone numbers / stable identifiers to redact
        pii_safe=True,
        ...
        platform_hint=(
            ...
            "plain text. Replies are not streamed — the user sees the whole reply when "
            "you finish. SimpleX has no typing indicator (a protocol limitation, not "
            "an adapter gap). Keep replies concise."
        ),
    )
    ...
    ctx.register_cli_command(
        ...
        setup_fn=add_simplex_subparser,
    )
