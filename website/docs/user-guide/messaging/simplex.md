...
The SimpleX Chat project publishes `simplexchat/simplex-chat` on Docker Hub; the "Run via Docker Compose" section below uses that image. If it is unavailable or you need a custom build, you can build from source from the [simplex-chat repository](https://github.com/simplex-chat/simplex-chat).
