...
def test_send_image_falls_back_to_text_without_bind_mount(monkeypatch):
    ...
    asyncio.run(adapter.send_image("group:42", "https://example.com/img.png"))
    payload = _sent_payload(adapter)
    # Text fallback delegates to send(), which emits the structured
    # /_send #<group_id> json [...] form (multi-line-safe).
    assert payload["cmd"] == "/_send #42 json " + json.dumps(
        [{"msgContent": {"type": "text", "text": "https://example.com/img.png"}}]
    )
