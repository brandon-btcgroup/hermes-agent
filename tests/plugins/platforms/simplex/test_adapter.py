...
from plugins.platforms.simplex.adapter import (
    SimplexAdapter,
    _apply_env,
    check_requirements,
    is_connected,
)
from plugins.platforms.simplex.client import SendResponse
...
def test_platform_enum_value():
    assert SIMPLEX.value == "simplex"


def test_check_requirements_returns_bool():
    """check_requirements only validates the websockets package is importable."""
    assert isinstance(check_requirements(), bool)


def test_is_connected_missing(monkeypatch):
...
def test_is_connected_present(monkeypatch):
...
def test_env_loading_roundtrip(monkeypatch):
    ...
    cfg.extra = {}
    _apply_env(cfg)
    assert cfg.extra["ws_url"] == "ws://daemon:5225"
    assert cfg.extra["group_ids"] == ["12", "13", "14"]
    assert cfg.extra["max_reconnect_delay_s"] == 120
    assert cfg.extra["file_dir"] == "/srv/simplex/files"
    assert cfg.extra["daemon_files_folder"] == "/data/simplex/files"
    assert cfg.home_channel.chat_id == "12"
    assert is_connected(cfg) is True


def test_is_connected_false_without_groups(monkeypatch):
    monkeypatch.setenv("SIMPLEX_WS_URL", "ws://daemon:5225")
    monkeypatch.delenv("SIMPLEX_GROUP_IDS", raising=False)
    cfg = PlatformConfig(enabled=True)
    cfg.extra = {}
    _apply_env(cfg)
    # ws_url is set but no group_ids — adapter is not "connected".
    assert is_connected(cfg) is False
...
async def test_dispatch_routes_groupRcv_text():
    ...
    assert ev.source.platform == SIMPLEX
...
# ── Plugin registration: SimpleX is a platform plugin, so cross-cutting
# wiring (send_message_tool, cron scheduler, gateway/run.py auth maps) is
# driven by the registry from register_platform() kwargs — no source-level
# assertions needed.  Discovery is exercised by test_plugin_discovery below.
...
async def test_pending_media_polls_and_dispatches_on_completion(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("plugins.platforms.simplex.adapter._FILE_POLL_INTERVAL_S", 0.01)
